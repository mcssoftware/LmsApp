...
const SPFieldLookupEdit: React.SFC<ISPFormFieldProps> = (props) => {
    let options: any[] = props.fieldSchema.Choices.map((option) => ({ key: option.LookupId, text: option.LookupValue }));
    if (props.fieldSchema.FieldType !== "LookupMulti") {
        if (!props.required) { options = [{ key: 0, text: FormFieldStrings.LookupEmptyOptionText }].concat(options); }
        const value: number = props.value ? Number(props.value.split(";#")[0]) : 0;
        return <Dropdown
        ...
        let values: any[] = [];
        if (props.value) {
            const splitArray: string[] = props.value.split(";#");
